...
describe('balanceDelta', () => {
  it('expense en efectivo resta', () => {
    expect(balanceDelta({ type: 'EXPENSE', accountType: 'CASH', role: 'SOURCE' })).toBe(-100);
  });
  it('income en débito suma', () => {
    expect(balanceDelta({ type: 'INCOME', accountType: 'DEBIT', role: 'SOURCE' })).toBe(100);
  });
  it('expense en crédito aumenta la deuda', () => {
    expect(balanceDelta({ type: 'EXPENSE', accountType: 'CREDIT', role: 'SOURCE' })).toBe(100);
  });
  it('transfer saliente resta en origen no-crédito', () => {
    expect(balanceDelta({ type: 'TRANSFER', accountType: 'DEBIT', role: 'SOURCE' })).toBe(-100);
  });
  it('transfer entrante en crédito reduce la deuda', () => {
    expect(balanceDelta({ type: 'TRANSFER', accountType: 'CREDIT', role: 'DESTINATION' })).toBe(-100);
  });
});
